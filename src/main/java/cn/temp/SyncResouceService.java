...
@Service
public class SyncResouceService {
        ...
        for (Label label : xlsBo.getLabelList()) {
            ...
            if (label.getRow() == 0) {
                if (!StringUtils.pdNULL(label.getContents().toString())) {
                    mapPara.put("loginEmail", label.getContents().toString());
                }
                ...
                if (!StringUtils.pdNULL(label.getContents().toString())) {
                    mapPara.put("postpaid", label.getContents().toString());
                }
            } else if (label.getRow() == 3) {
                if (!StringUtils.pdNULL(label.getContents().toString())) {
                    mapPara.put("loginName", label.getContents().toString());
                }
            } else if (label.getRow() == 4) {
                if (!StringUtils.pdNULL(label.getContents().toString())) {
                    mapPara.put("mobilephone", label.getContents().toString());
                }
            } else if (label.getRow() == 5) {
                if (!StringUtils.pdNULL(label.getContents().toString())) {
                    mapPara.put("fkRootUserid", label.getContents().toString());
                }
            } else if (label.getRow() == 6) {
                if (!StringUtils.pdNULL(label.getContents().toString())) {
                    mapPara.put("isOnDemandAccount", label.getContents().toString());
                }
            }
...
//            System.out.print(i++);
    ...
    private static String doReport(HttpRest httpRest, Map<String, String> map, int i) {
        String s = "";
        try {
            s = httpRest.postParamAsMap("http://10.144.242.126:80/hwservice/createIAMUser", map, null, null);
    ...
    public static void main(String[] args) {
        getHost("C:\\Users\\22984\\Desktop\\a.xls");
    }
}
